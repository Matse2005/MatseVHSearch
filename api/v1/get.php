...
if (!isset($data["index"]) || !isset($data["app_id"]) || !isset($data["key"])) {
  // If not, return a 400 error
  http_response_code(400);
  echo json_encode(array("message" => "Bad request", "specific_message" => "The request must contain the following parameters: index, app_id, key"));
  exit();
}
...
$objects = fread($file, filesize($_SERVER["DOCUMENT_ROOT"] . "/documents/" . $data["app_id"] . "/" . $data["index"] . ".json"));
// Close the file
fclose($file);

// Return a 200 success
http_response_code(200);
echo json_encode(array("message" => "Success", "objects" => json_decode($objects)));
exit();
